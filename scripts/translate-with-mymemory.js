...
const USER_EMAIL = 'ernesto.roberts.707@gmail.com'; // Increases limit to 50,000 chars/day
const DELAY_BETWEEN_REQUESTS = 1500; // 1.5 seconds to respect rate limits
...
async function translateText(text, retries = 3) {
    ...
    const MAX_CHARS = 450; // Leave some buffer below 500 char limit
...
async function translateChunk(text, retries = 3) {
    const url = `${MYMEMORY_API_URL}?q=${encodeURIComponent(text)}&langpair=${SOURCE_LANG}|${TARGET_LANG}&de=${encodeURIComponent(USER_EMAIL)}`;
...
// Main function
async function main() {
    console.log('🌍 MyMemory Translation Script');
    console.log('================================\n');
    ...
    console.log('\nNext steps:');
    console.log('1. Review the translated content');
    console.log('2. Copy the translated files to the new English project');
}
