...
const TextHighlighter = () => {
  ...
  const findFunction = (text) => {
    ...
    // Get the appropriate substrings set based on varClave
    const substrings = substringsFn[varClave] || {};

    // Create a regex pattern to match any of the substrings
    const pattern = new RegExp(Object.keys(substringsFn).join('|'), 'gi');
    ...
    const highlightedText = textWithSpaces.split(pattern).reduce((acc, part, index, arr) => {
      if (index < arr.length - 1) {
        const matchFn = textWithSpaces.match(pattern)[index];
        const FnArm = substrings[match];

        console.log(matchFn);
        console.log(FnArm);        
        
        acc.push(part, <span key={index}>{matchFn}</span>);
      } else {
        acc.push(part);
      }


      if (varClave === 'C'  &&  match === 'Bb') { };
